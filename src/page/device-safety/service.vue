...
<script>
    import { DivisionService } from '@/api/hidden-trouble' // api
    ...
    export default {
        data() {
            ...
            return {
                ...
                divisionFormRules: { // 表单校验
                    fBeginTime: [
                        {required: true, validator: CDstartTime, trigger: 'change'},
                    ],
                    fEndTime: [
                        {required: true, validator: CDendTime, trigger: 'change'}
                    ],
                    fName: [
        ...
        methods: {
             ...
             */
            async getDivision(){
                let res = await DivisionService.getDivision();
                if (res.success) {
            ...
            deleteDivision(row){
                ...
                this.$confirm(`此操作将删除当前数据，是否继续？`, '提示', {
                    ...
                    let res = await DivisionService.deleteDivision(row.fId);
                        if (res.success) {
            ...
            async insertDivisionForm(){
                let res = await DivisionService.insertDivision(this.divisionForm);
                if (res.success) {
            ...
            async updateDivisionForm(){
                let res = await DivisionService.updateDivision(this.divisionForm);
                if (res.success) {
                    this.closeModal('divisionForm',this.divisionModal);
                    this.$message.success(res.msg);
                    this.getDivision();
