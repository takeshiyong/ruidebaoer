<template>
    <div class="page video-container">
            ...
            <el-col :lg="5" :md="7" :sm="9" :xs="24">
                <div class="left-div">
                    <div class="left-div-header">
                        <span>视频监控列表</span>
                        <el-tooltip effect="dark" content="重启easyNvr" placement="top" >
                            <el-button type="text" icon="otherredoublefonts redouble-loop" class="opearBtn ml10 mt-4" @click="restartOpe('1')"></el-button>
                        </el-tooltip>
                        <el-tooltip v-if="areaDto.fId || (areaDtoTemp && areaDtoTemp.fId)" effect="dark" :content="delTitle" placement="top">
                            <el-button type="text" icon="el-icon-delete" class="opearBtn ml10 mt-4" @click="handleDel"></el-button>
                        </el-tooltip>
                        <el-tooltip v-if="areaDto.fId || (areaDtoTemp && areaDtoTemp.fId)" effect="dark" :content="editTitle" placement="top">
                            <el-button type="text" icon="el-icon-edit" class="opearBtn mt-4" @click="handleEdit"></el-button>
                        </el-tooltip>
                        <el-tooltip effect="dark" :content="addTitle" placement="top">
                    ...
                    <div style="display: flex;marginTop: 8px;marginBottom: 5px;marginRight: 20px;marginLeft: 20px">
                        ...
                        <el-button type="primary" icon="el-icon-search" style="marginLeft: 5px" @click="getVideoArea('','',true)"></el-button>
                    </div>
                    <div class="left-div-header" style="height: calc(100vh - 236px);overflowY: auto;width:calc(100% - 40px);paddingTop: 0px;paddingBottom: 0px;">
                        <el-button icon="el-icon-plus" style="width: 100%" @click="initAddType()"></el-button>
                    ...
                    <el-tabs v-model="tabsName" ref="tabs" type="border-card" style="overflowY: hidden;box-shadow: none;">
                        <el-tab-pane name="resource" label="视频展示资源管理">
                            <div v-if="tabsName ==='resource'" class="right-div">
                                <el-tooltip class="tip-icon" effect="dark" content="新建视频展示资源" placement="top" >
                                    <el-button type="text" icon="el-icon-plus" class="opearBtn mt-4" @click="createNewVideo"></el-button>
                                </el-tooltip>
                                <div class="content-body">
                                    <div class="content-video" style="overflowY: auto">
                                        <draggable @end="videoListEnd" @start="videoListSort" @add="dragEnd" class="content-video" v-model="videoList" :options="{group:{name: 'people', put: 'baz', pull: ['people']}, sort: true}">
                                            <div v-for="(item, key) in videoList" :key="item.fId" class="video-item">
                                                <div class="video-content">
                                                    ...
                                                    <div v-else-if="!item.fIsCycle && item.fCurrentShowVido" style="width:100%;height: 94%;backgroundColor: #000">
                                                        <singlePlayVideo playType="rtmp" type="small" :channel="item.fCurrentShowVidoChanel" ></singlePlayVideo>
                                                    </div>
                                                    <div v-else-if="item.fIsCycle && item.fChannels.length > 0" style="width:100%;height: 94%;backgroundColor: #000">
                                                ...
                                                <div class="sub-prompt">
                                                    ...
                                                    <div class="btn-group">
                                                        <el-tooltip effect="dark" :content="item.fIsCycle?'循环播放':'非循环播放'" placement="top" >
                                                            <i :class="`redoublefont ${item.fIsCycle?'redouble-loop':'redouble-no-loop'}`"  style="fontSize: 12px"></i>
                                                        </el-tooltip>
                                                        <el-tooltip effect="dark" :content="'推送大屏'" placement="top">
                                                            <i class="redoublefont redouble-iconcopy click" style="fontSize: 12px;marginLeft:5px" @click="pushScreen(item, 1)"/>
                                                        </el-tooltip>
                                                        <i v-if="item.fChannels.length > 0" class="redoublefont redouble-quanping click" style="fontSize: 16px" @click="handleCommand('size', item)"/>
                                                        <i class="redoublefont redouble-shezhi click" style="fontSize: 16px" @click="handleCommand('update', item)"/>
                                                        <i class="el-icon-delete click" style="fontSize: 14px" @click="handleCommand('del', item)"/>
                                                    </div>
                                                </div>
                                            </div>
                                        </draggable>
                        ...
                        <el-tab-pane name="video" v-if="showVideoPlay" label="监控视频" style="height:100%">
                            <span slot="label">
                                视频监控 <i class="el-icon-close" @click="clickCloseVideo"></i>
                            </span>
                            <div v-if="tabsName == 'video' && fStreamType" style="position: absolute;top: -28px; right: 10px;">
                                <el-switch
                                    v-if="fStreamType"
                                    style="display: block"
                                    v-model="fStreamType"
                            ...
                            <div class="right-div" style="overflow: hidden" :key="keys">
                                <singlePlayVideo playType="flv" v-if="videoType==='single'" :fStreamType="fStreamType" :fRealityChannel="fRealityChannel" :channel="playChannelId" isControl='true' :ptaControl="ptaControl" :nvrIP="fIp" ></singlePlayVideo>
        ...
        <TheDialog :modal='videoVisible' :modalTitle="videoTitle" @closeModal="videoVisible = false">
                                ...
                                <el-input v-model="videoDto.fUrl" readonly disabled>
                                    <div slot="suffix" class="icon-tip" @click="handlePaste">
                                        <el-tooltip effect="dark" :content="'复制'" placement="top">
                                            <i class="redoublefont redouble-paste" style="color: #333"></i>
                                        </el-tooltip>
                                    </div>
                                </el-input>
                            </el-form-item>
        ...
        <TheDialog :modal='areaVisible' :modalTitle="areaTitle" @closeModal="closeTypeAdd()">
            <div slot="custom-modal-content">
                <el-form ref="areaRef" :model="areaDto" :rules="areaRules" label-width="110px">
                    <el-row :gutter="8">
                        <el-col :span="12">
                            <el-form-item label="分类名称" prop="fName">
                                <el-input v-model="areaDto.fName" placeholder="请填写分类名称" clearable></el-input>
                            </el-form-item>
                        </el-col>
                        <el-col :span="12" v-if="areaTitle== '新建分类' && !areaDto.fId && areaDto.fParentId == '-1'">
                            <el-form-item label="分类类型" prop="fName">
                                 <el-radio-group v-model="areaDto.fType">
                                    <el-radio :label="1">摄像头分类
                                            <i class="el-icon-info" title="只能添加摄像头"></i>
                                    </el-radio>
                                    <el-radio :label="2">子分类
                                            <i class="el-icon-info" title="只能添加子分类"></i>
                                    </el-radio>
                                </el-radio-group>
                            </el-form-item>
                        </el-col>
        ...
        <TheDialog :modal='channelVisible' :modalTitle="channelTitle" @closeModal="channelVisible = false">
            <div slot="custom-modal-content">
                <el-form ref="channelRef" :model="channelDto" :rules="channelRules" label-width="110px">
                    <el-row :gutter="8">
                        <el-col :span="12">
                            <el-form-item label="区域" title="区域" prop="fAreaId">
                                <areaSelect :width="247.5" :value="channelDto.fAreaId" @change="changeChannelArea"></areaSelect>
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item label="Ip地址" title="Ip地址"  prop="fIp">
                                <!-- <el-input v-model="channelDto.ip" placeholder="请填写Ip地址"></el-input> -->
                                <el-select filterable v-model="channelDto.fIp" clearable placeholder="请选择Ip地址" style="width: 100%">
                                    <el-option
                        ...
                        <el-col :span="12">
                            <el-form-item label="摄像机名称" title="摄像机名称" prop="fName">
                                <el-input v-model="channelDto.fName" placeholder="请填写摄像机名称"></el-input>
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item label="nvr真实通道" title="nvr真实通道" prop="fRealityChannel">
                                <el-input v-model="channelDto.fRealityChannel" placeholder="请填写nvr真实通道"></el-input>
                            </el-form-item>
                        </el-col>
                        <el-col v-if="!channelDto.fId" :span="12">
                            <el-form-item  label="码流类型" title="码流类型" prop="fStreamType">
                                <el-select filterable v-model="channelDto.fStreamType" clearable placeholder="请选择码流类型" style="width: 100%">
                        ...
                        <el-col :span="12">
                            <el-form-item label="是否启用"   prop="fEnable">
                                <el-switch
                                    style="marginTop: -2px"
                                    v-model="channelDto.fEnable"
                                    :active-value="true"
                        ...
                        <el-col :span="!channelDto.fId ? 24 : 12">
                            <el-form-item label="是否控制" prop="fPtzControl">
                                <el-switch
                                    style="marginTop: -2px"
                                    v-model="channelDto.fPtzControl"
                                    :active-value="true"
                                    :inactive-value="false"
                                    active-text="是"
                                    inactive-text="否">
        ...
        <TheDialog :modal='outsideVisible' width="950px" :modalTitle="'对外展示资源配置'" @closeModal="outsideVisible = false">
            <div slot="custom-modal-content">
...
<script>
    // api
    import { videoService, cameraService, videoAreaService, channelService, resourceService } from '@/api/video';
    import { CatalogService } from '@/api/personal-info' // api
    import { nvrEdit } from '../../api/nvrEdit';
    import { collectRest } from '../../api/common';
    import tree from '../../components/videoComponent/tree.vue'
    ...
    import operationVideo from '../../components/videoComponent/operationVideo.vue';
    ...
    export default {
        data(){
            ...
            return {
                ...
                addTitle: '新建分类', // 新建按钮标题
                editTitle: '编辑分类', // 编辑按钮标题
                delTitle: '删除分类', // 删除按钮标题
                pageParam: {
                    currentPage: 1,
                    sum: 0,
                    pageSize: 4
                ...
                channelDto: {
                    ...
                    fIp: '',             // ip地址id
                    fId: '',             // id
                    fPtzControl: false,  // 是否云控制
                    fRealityChannel: '', // 真实通道号
                    fStreamType: 1,      // 码流类型 1：主码流 2：子码流
                    fName: '',           // 摄像头名称
                    fVideoAreaId: '',    // 摄像头所属分类id
                },
                tempChannel: {},
                updateId: '',
                showVideoPlay: false,
        ...
        components: {
            TheDialog, tree, draggable, areaSelect,
            channelType,singlePlayVideo, multPlayVideo,
            operationVideo
        },
        computed: {
       ...
       //console.log('this.resourceTree',this.resourceTree)
            ...
            async getDefaultTime() {
                const res = await CatalogService.getDictionaryContexts('spbfcs');
                if (res.success) {
                    for(let item of res.obj) {
            ...
            showVideoUrl(url) {
                ...
                this.fIp = url.fIp;
                this.fStreamType = url.fStreamType || '';
                this.showVideoPlay = true;
                this.tabsName = 'video';
                this.fVideoId = url.fId;
            ...
            // 切换码流
            async changeStreamType(e) {
                console.log('切换码流', e);
                const res = await channelService.changeSteamType({
                    fVideoSteam: e,
                ...
                if (res.success) {
                    ...
                    // }, 4000)
                    this.limit = 0;
                    this.cycTimesStream = setInterval(()=>{
                        if (this.limit && this.limit == 10) {
                            if (this.cycTimesStream) {
                                clearInterval(this.cycTimesStream);
                            }
                        }
                        this.limit = this.limit++;
                        this.checkChannelOnline(this.playChannelId);
                    }, 500);
                    this.getVideoArea();
                    this.fStreamType = e;
                } else {
            ...
            // 判断摄像头在线状态
            async checkChannelOnline(channel) {
                const res = await channelService.checkOnline(channel);
                if (res.success) {
                    if (res.obj.Online == 1) {
                        this.keys = this.keys+1;
                        if (this.cycTimesStream) {
            ...
            // 摄像头修改
            async updateChannel() {
                if (this.channelDto.treeData) {
                    delete this.channelDto.treeData
                }
            ...
            // 查询分类数据
            async getVideoArea(id, isDel, isSearch) {
                let firstArr = [];
                let secondArr = [];
                ...
                for (let [index, item] of this.videoArr.entries()) {
                    ...
                    if (item.fVideoChannelConfigList && item.fVideoChannelConfigList.length > 0) {
                        for (let [idx, obj] of item.fVideoChannelConfigList.entries()) {
                            if (obj.current) {
                                currentObj.first = index;
                ...
                const res = await videoAreaService.select(this.searchName);
                if (res.success) {
                    if (isSearch) {
                        this.channelDto = {
                            fAreaId: '',         // 区域选择
                    ...
                    this.videoArr = res.obj;
                    this.handleVidredoubleonfigList(id, currentObj, firstArr, secondArr, isDel);
                    console.log(this.videoArr, 'this.vide')
                } else {
                    this.$message.warning(res.msg);
                }
            },
            handleVidredoubleonfigList(id, currentObj, firstArr, secondArr, isDel) {
                ...
                if (currentObj.third == -1 && currentObj.second != -1) {
                    if (this.videoArr[currentObj.first] && this.videoArr[currentObj.first].fVideoChannelConfigList.length > 0) {
                        this.videoArr[currentObj.first].fVideoChannelConfigList[currentObj.second].current = true;
                    } else {
                ...
                for (let [index, item] of this.videoArr.entries()) {
                    ...
                    if (item.fVideoChannelConfigList && item.fVideoChannelConfigList.length > 0) {
                        for (let [key, val] of item.fVideoChannelConfigList.entries()) {
                            if (this.updateType === 'channel' && val.fId === this.channelDto.fId) {
                                this.tempChannel = val;
                            }
                            if (val.fId === this.updateId) {
                                val.current = true;
                                this.channelDto = {
                                    ...this.channelDto,
                                    ...val,
            ...
            handleAdd() {
                if (this.addTitle === '新建摄像头') {
                    console.log('this.areaDto', this.areaDto);
                    this.getIp();
                    ...
                    this.channelDto = {
                        ...
                        fStreamType: 1,      // 码流类型 1：主码流 2：子码流
                        fName: '',           // 摄像头名称
                        fVideoAreaId: this.areaDto.fId,    // 摄像头所属分类id
                    };
                    return;
                }
                ...
                this.areaTitle = '新建分类';
                if (this.areaDto.fId) {
                    this.areaDtoTemp = this.areaDto;
                }
                this.areaDto = {
                    fName: '',
                    fType: 1,
                    fParentId: this.areaDtoTemp.fId,
                    fId: ''
                }
            },
            // 关闭分类弹窗
            closeTypeAdd() {
                ...
                if (this.areaDtoTemp) {
                    this.areaDto = this.areaDtoTemp;
                }
                this.areaDtoTemp = null;
            },
            // 新建一级分类
            initAddType () {
                this.areaVisible = true;
            ...
            // 编辑分类
            handleArea() {
                if (this.areaTitle === '新建分类') {
                    this.insertArea();
                    return;
                }
                if (this.areaTitle === '编辑分类') {
                    this.areaUpdate();
                }
            },
            handlePaste() {
            ...
            toggleExpends(item, index, idx, i) {
                console.log('item', item, index, idx, i);
                // 递归处理展开
                if (idx != null || item.fParentId != null) {
                    if (!item.fIp) {
                        this.areaDto = {...item};
                        this.editTitle = '编辑分类';
                        this.delTitle = '删除分类';
                ...
                if (idx != null) {
                    console.log('走了这里啊', index, idx, this.videoArr[index].fVideoAreaList);
                    if (this.videoArr[index].fVideoAreaList && this.videoArr[index].fVideoAreaList[idx]) {
                        ...
                        console.log(this.videoArr[index].fVideoChannelConfigList, '123123123')
                        this.videoArr[index].fVideoChannelConfigList[idx].current = true;
                    }
                    this.$forceUpdate();
                    return;
                }
            ...
            recursion(arr, obj) {
                for (let item of arr) {
                    item.current = false;
                    if (item.fVideoChannelConfigList && item.fVideoChannelConfigList.length > 0) {
                        for (let obj of item.fVideoChannelConfigList) {
                            obj.current = false;
                        }
                        continue;
                    }
                    if (!item.fVideoAreaList) continue;
            ...
            judgeAddTitle(item) {
                ...
                } else if (item.fParentId != null) {
                    // 区域数据
                    if (item.fType == null || item.fType == 1) {
                        this.addTitle = '新建摄像头';
                    } else {
                        this.addTitle = '新建分类';
                    }
